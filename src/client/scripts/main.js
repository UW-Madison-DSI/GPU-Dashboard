...
function fetchOlderStorageData(options, min_age) {
	$.ajax({
		url: server + '/storage/older/' + min_age,
		type: 'GET',
		crossDomain: true,
		headers: {
			"Access-Control-Allow-Origin": "*"
...
function fetchAllStorageData(options) {
	let array = [];
...
function showGpuUsageCharts(data) {
	let olvi1Data = getDataByHost(data, 'olvi-1');
	let olvi2Data = getDataByHost(data, 'olvi-2');

	// show charts
	//
	let olvi1Chart = new GpuUsageChart({
		title: 'Olvi-1 GPU Usage',
		element: $('#olvi-1-gpu-usage .chart')[0],
		data: olvi1Data
	});
	let olvi2Chart = new GpuUsageChart({
		title: 'Olvi-2 GPU Usage',
		element: $('#olvi-2-gpu-usage .chart')[0],
		data: olvi2Data
	});
	...
	if (olvi1Data.length > 0) {
		...
		this.showDate($('#olvi-1-gpu-usage .updated'), date);
	}
	if (olvi2Data.length > 0) {
		let date = new Date(olvi2Data[0].created_at + 'Z');
		this.showDate($('#olvi-2-gpu-usage .updated'), date);
	}
}
...
function showSelectedStorageCharts(data) {
	let selected = parseInt($('.selector').val());
	let olvi1Data = getDataByHost(data[selected], 'olvi-1');
	let olvi2Data = getDataByHost(data[selected], 'olvi-2');
...
function showProcessCharts(data) {
	this.fetchProcessData({

		// callbacks
		//
		success: (data) => {
			showGpuUsageCharts(data);
...
function showStorageCharts(data) {
	this.fetchAllStorageData({

		// callbacks
		//
		success: (data) => {
			storage = data
			showSelectedStorageCharts(data);
		}
	});
}
...
window.onload = function() {
	...
	$('a#gpu-memory').click(() => {
		window.location.hash = 'gpu-memory';
	});
	$('a#gpu-load').click(() => {
	...
	// ser selector callback
	//
	$('.selector').change(() => {
		this.showSelectedStorageCharts(storage);
	});
